...
export class MatrixSolver {
  ...
  static solveByInverse(coefficients: number[][], constants: number[]): MatrixSolution {
    const steps: string[] = [];
    ...
    steps.push(`Determinante de A = ${det}`);
  ...
  // Método de Cramer
  static solveByCramer(coefficients: number[][], constants: number[]): MatrixSolution {
    const steps: string[] = [];
    const n = coefficients.length;
    ...
    steps.push(`Determinante principal = ${mainDet}`);
    ...
    for (let i = 0; i < n; i++) {
      ...
      steps.push(`x${i + 1} = Det(A${i + 1}) / Det(A) = ${det} / ${mainDet} = ${variable}`);
    }
  ...
  static solveByGaussJordan(coefficients: number[][], constants: number[]): MatrixSolution {
    const steps: string[] = [];
    const n = coefficients.length;
    ...
    for (let i = 0; i < n; i++) {
      ...
      steps.push(`Dividiendo fila ${i + 1} por ${pivot}`);
      steps.push(this.matrixToString(augmented));
      ...
      for (let k = 0; k < n; k++) {
        if (k !== i && Math.abs(augmented[k][i]) > 1e-10) {
          ...
          steps.push(`F${k + 1} = F${k + 1} - ${factor} * F${i + 1}`);
          steps.push(this.matrixToString(augmented));
        }
      }
    }
  ...
  // Utilidades
  private static calculateDeterminant(matrix: number[][]): number {
  ...
  private static calculateInverse(matrix: number[][]): number[][] {
    const n = matrix.length;
  ...
  private static matrixToString(matrix: number[][]): string {
    return matrix.map(row => 
